...
void Arduino_RGB_Display::cache_writeback(void* addr, size_t size)
{
  esp_cache_msync(addr, size, ESP_CACHE_MSYNC_FLAG_UNALIGNED);
}
  ...
  { // Y on screen, nonzero width
    ...
    { // Not off right
      ...
      { // Not off left
        ...
        if (_auto_flush)
        {
          cache_writeback(cachePos, writeSize);
        }
      }
    }
  }
                                                  ...
                                                  int16_t w, int16_t h, uint16_t color)
{
  ...
  if (_auto_flush)
  {
    cache_writeback(cachePos, _fb_width * h * 2);
  }
}
                                             ...
                                             uint16_t *bitmap, int16_t w, int16_t h)
{
  ...
  if (result)
  {
    if (_auto_flush)
    {
      ...
      cache_writeback(cachePos, cache_size);
    }
  }
}
...
void Arduino_RGB_Display::flush(void)
{
  if (!_auto_flush)
  {
    cache_writeback(*_framebuffer, _framebuffer_size);
  }
}
