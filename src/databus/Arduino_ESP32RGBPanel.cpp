...
// experimental option to use two frame buffere in PSRAM instead of one
//#define USE_TWO_FRAMEBUFFERS // doesn't work this way

#if defined(USE_TWO_FRAMEBUFFERS)
bool Arduino_ESP32RGBPanel::on_vsync_event(esp_lcd_panel_handle_t panel, const esp_lcd_rgb_panel_event_data_t *event_data, void* user_data)
{
  Arduino_ESP32RGBPanel* me = reinterpret_cast<Arduino_ESP32RGBPanel*>(user_data);
  void* t = me->frame_buffer_1;
  me->frame_buffer_1 = me->frame_buffer_2;
  me->frame_buffer_2 = t;
  return false;
}
#endif
...
uint16_t** Arduino_ESP32RGBPanel::getFrameBuffer(int16_t w, int16_t h)
{
  ...
  ESP_ERROR_CHECK(esp_lcd_rgb_panel_get_frame_buffer(_panel_handle, 2, &frame_buffer_1, &frame_buffer_2));
  if (frame_buffer_2 == NULL) frame_buffer_2 = frame_buffer_1; // oops, shouldn't occur
#else
  ESP_ERROR_CHECK(esp_lcd_rgb_panel_get_frame_buffer(_panel_handle, 1, &frame_buffer_1));
  frame_buffer_2 = frame_buffer_1; // just in case
#endif

  return (uint16_t **) &frame_buffer_1;
}
