...
Arduino_UNOPAR8_WRS::Arduino_UNOPAR8_WRS()
{
}

bool Arduino_UNOPAR8_WRS::begin(int32_t, int8_t)
{
...
void Arduino_UNOPAR8_WRS::beginWrite()
{
  DC_HIGH();
  CS_LOW();
}

void Arduino_UNOPAR8_WRS::endWrite()
{
  CS_HIGH();
}

void Arduino_UNOPAR8_WRS::writeCommand(uint8_t c)
{
  DC_LOW();
...
void Arduino_UNOPAR8_WRS::writeCommand16(uint16_t c)
{
  DC_LOW();
...
void Arduino_UNOPAR8_WRS::write(uint8_t d)
{
  WRITE(d);
}

void Arduino_UNOPAR8_WRS::write16(uint16_t d)
{
  _data16.value = d;
  WRITE(_data16.msb);
  WRITE(_data16.lsb);
...
void Arduino_UNOPAR8_WRS::writeRepeat(uint16_t p, uint32_t len)
{
...
void Arduino_UNOPAR8_WRS::writePixels(uint16_t *data, uint32_t len)
{
  while (len--)
  {
    _data16.value = *data++;
...
void Arduino_UNOPAR8_WRS::writeC8D8(uint8_t c, uint8_t d)
{
  DC_LOW();
...
void Arduino_UNOPAR8_WRS::writeC8D16(uint8_t c, uint16_t d)
{
  DC_LOW();
...
void Arduino_UNOPAR8_WRS::writeC8D16D16(uint8_t c, uint16_t d1, uint16_t d2)
{
  DC_LOW();
...
void Arduino_UNOPAR8_WRS::writeBytes(uint8_t *data, uint32_t len)
{
  while (len--)
  {
    WRITE(*data++);
...
void Arduino_UNOPAR8_WRS::writeIndexedPixels(uint8_t *data, uint16_t *idx, uint32_t len)
{
  while (len--)
  {
...
void Arduino_UNOPAR8_WRS::writeIndexedPixelsDouble(uint8_t *data, uint16_t *idx, uint32_t len)
{
  while (len--)
  {
    _data16.value = idx[*data++];
...
INLINE void Arduino_UNOPAR8_WRS::WRITE(uint8_t d)
{
  volatile int wr_stretch = 0;
#if defined(ARDUINO_AVR_UNO)
...
INLINE void Arduino_UNOPAR8_WRS::DC_HIGH(void)
{
#if defined(ARDUINO_AVR_UNO)
  PORTC |= _BV(2); // RS_H;
#elif defined(ARDUINO_AVR_MEGA2560)
...
INLINE void Arduino_UNOPAR8_WRS::DC_LOW(void)
{
#if defined(ARDUINO_AVR_UNO)
  PORTC &= ~_BV(2); // RS_L;
#elif defined(ARDUINO_AVR_MEGA2560)
...
INLINE void Arduino_UNOPAR8_WRS::CS_HIGH(void)
{
#if defined(ARDUINO_AVR_UNO)
  PORTC |= _BV(3); // CS_H;
#elif defined(ARDUINO_AVR_MEGA2560)
...
INLINE void Arduino_UNOPAR8_WRS::CS_LOW(void)
{
#if defined(ARDUINO_AVR_UNO)
  PORTC &= ~_BV(3); // CS_L;
#elif defined(ARDUINO_AVR_MEGA2560)
